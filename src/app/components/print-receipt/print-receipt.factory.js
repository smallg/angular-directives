...
(function () {
    ...
    angular
        .module('jfy')
        .factory('PrintReceipt', PrintReceipt);

    function PrintReceipt($window, toastr) {
        var _window;
        var popupWindow = function (data, delayTime) {
            var drugList = '';
            ...
            var customScript = '<script>function printAndClose(){window.print();setTimeout(function(){window.close();},' + delayTime + ')}</script>';
            ...
            _window = $window.open('', '_blank', 'width=600,height=700,scrollbars=no,menubar=no,toolbar=no,location=no,status=no,titlebar=no');
            _window.document.open();
            _window.document.write('<html><head>' + customStyles + '</head><body onload="printAndClose();">' + innerContents + customScript + '</body></html>');
            _window.document.close();
        };
        return {
            /* 80mm thermal paper or A5 paper */
            printX: function (data, delayTime) {
                toastr.info("处方笺打印中……");
                popupWindow(data, delayTime);
            }
        }
    }
})();